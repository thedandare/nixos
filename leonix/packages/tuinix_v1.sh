...
#   ];
#
# and lets you enable/disable them through gum menus.
#
# Requirements:
#   - bash 4+
#   - gum
...
#
# Design notes:
#   - Gum has no native tabs. TUINIX simulates file tabs through a header.
...
require_dependencies() {
  if ! has_cmd gum; then
    cat >&2 <<'MSG'
TUINIX requires gum, but gum was not found in PATH.

On Nix/NixOS you can try:

  nix shell nixpkgs#gum

or add `gum` to your environment.systemPackages.
MSG
    exit 127
  fi
...
style_warn() {
  gum style --bold --foreground 11 "$*"
}

pause() {
  printf '\n'
  gum input --placeholder "Enter para continuar..." >/dev/null || true
}
...
choose_one() {
  local prompt="$1"
  shift
  gum choose --height "$TUINIX_HEIGHT" --cursor-prefix "> " --header "$prompt" "$@"
}

choose_multi() {
  local prompt="$1"
  shift
  gum choose --no-limit --height "$TUINIX_HEIGHT" --cursor-prefix "> " --header "$prompt" "$@"
}

filter_one() {
  local prompt="$1"
  gum filter --height "$TUINIX_HEIGHT" --placeholder "$prompt"
}
...
ui_edit_section_packages() {
  ...
  render_header "Marque/desmarque pacotes: $(basename_safe "$file") > $section"
  render_tabs "$file"

  local selected
  selected="$(gum choose --no-limit --height "$TUINIX_HEIGHT" --cursor-prefix "> " --header "Espaço/Enter para selecionar" "${selected_args[@]}" "${options[@]}")" || return
