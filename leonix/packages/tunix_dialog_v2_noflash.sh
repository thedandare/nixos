...
set -Euo pipefail
# NOTE: do not use `set -e` in dialog/ncurses loops. Many dialog actions
# intentionally return non-zero status codes (Cancel/Esc/Help/Extra), and
# letting errexit leak into UI code makes the program close as soon as the
# user presses Enter/Space in some terminals.
# O que cada flag faz
# -e   errexit
# -E   errtrace
# -u   nounset
...
TUINIX_NAME="TUINIX"
TUINIX_VERSION="2.2.0-dialog-v2"
TUINIX_TMPDIR=""
...
# =============================================================================
# TUINIX dialog V2 overrides
# =============================================================================
# This block intentionally overrides selected functions from the v2.1 monolith.
# Main changes:
#   - selecting a .nix opens the package checklist first;
#   - actions are available from the same checklist via the Extra button;
#   - NixOS Search is available from package focus/help where dialog supports it,
#     with graceful fallback to a package picker;
#   - ESC/CANCEL always means "go back" instead of exiting;
#   - Unicode is preserved by default, while --no-tags reduces dialog rendering
#     glitches with hidden numeric tags. ASCII fallback is opt-in.
#   - optional tmux side preview can be enabled with TUINIX_TMUX_SIDE=1.

TUINIX_VERSION="2.2.0-dialog-v2"
TUINIX_NIXOS_CHANNEL="${TUINIX_NIXOS_CHANNEL:-26.05}"
TUINIX_ASCII_UI="${TUINIX_ASCII_UI:-0}"
TUINIX_TMUX_SIDE="${TUINIX_TMUX_SIDE:-0}"
TUINIX_TMUX_PANE_ID=""
TUINIX_TMUX_PREVIEW_FILE=""

# Hide internal numeric tags in dialog lists. This alone makes the checklist much
# cleaner and avoids the coloured 000/001/line-id column that appeared in v2.1.
DIALOG_COMMON=(--stdout --clear --mouse --backtitle "$TUINIX_NAME $TUINIX_VERSION")

setup_ui_environment() {
  # Do not force the whole process to a different locale, but make a sensible
  # UTF-8 fallback when the shell is running in C/POSIX. dialog/ncurses need a
  # UTF-8 locale to compute wide-character cell widths correctly.
  ...
  # Prefer Unicode line drawing from the terminal font instead of ncurses ACS
  # approximations. This helps on several Linux terminal emulators.
  export NCURSES_NO_UTF8_ACS="${NCURSES_NO_UTF8_ACS:-1}"
}

# Override init/cleanup so tmux preview pane is cleaned up if it was created.
init_runtime() {
  setup_ui_environment
  TUINIX_TMPDIR="$(mktemp -d -t tuinix.XXXXXX)"
  TUINIX_TMUX_PREVIEW_FILE="$TUINIX_TMPDIR/tmux-preview.txt"
  trap cleanup EXIT
}

cleanup() {
  local code=$?
  tmux_preview_stop || true
  if [[ -n "${TUINIX_TMPDIR:-}" && -d "${TUINIX_TMPDIR:-}" ]]; then
    rm -rf -- "$TUINIX_TMPDIR"
  fi
  exit "$code"
...
# Capture dialog safely under set -e. Return code is preserved:
#   0 OK, 1 Cancel, 2 Help, 3 Extra, 4 Item-help, 255 ESC.
run_dialog_capture() {
  local __out_var="$1"
  shift
  local __out __rc
  set +e
  __out="$(dialog "${DIALOG_COMMON[@]}" "$@")"
  __rc=$?
  # keep errexit disabled globally for dialog-driven control flow
  printf -v "$__out_var" '%s' "$__out"
  return "$__rc"
}

run_dialog_nocapture() {
  local __rc
  set +e
...
confirm() {
  local msg="$1"
  run_dialog_nocapture --yesno "$msg" 8 70
}

pause() {
  run_dialog_nocapture --msgbox "Pressione Enter para continuar." 7 50 >/dev/null || true
}

pager_text() {
  local title="$1"
  local file="$2"
  run_dialog_nocapture --title "$title" --textbox "$file" 0 0 || true
}
...
input_text() {
  ...
  local initial="${2:-}"
  local out
  run_dialog_capture out --inputbox "$prompt" 8 70 "$initial" || return $?
  printf '%s' "$out"
}
...
choose_one() {
  ...
  for item in "$@"; do
    tag="$(printf '%03d' "$i")"
    printf '%s\t%s\n' "$tag" "$item" >> "$tmp_items"
    args+=("$tag" "$(ui_safe "$item")")
    ((i++)) || true
  done

  [[ ${#args[@]} -gt 0 ]] || return 1

  local tag_selected rc
  if run_dialog_capture tag_selected \
    --no-tags \
    --cancel-label "Voltar" \
    --menu "$(ui_safe "$prompt")" "$TUINIX_HEIGHT" "$TUINIX_WIDTH" $(( TUINIX_HEIGHT - 8 )) \
    "${args[@]}"; then
    rc=0
  else
    rc=$?
  ...
  case "$rc" in
    0)
      # Depending on dialog/ncurses version, --no-tags may return either the
      # hidden tag or the visible item text. Accept both.
      local resolved
      resolved="$(awk -F '\t' -v t="$tag_selected" '$1 == t { print substr($0, length($1) + 2); found=1; exit } END { if (!found) exit 1 }' "$tmp_items")" || resolved="$tag_selected"
      printf '%s\n' "$resolved"
      ;;
    1|255) return 1 ;;
    *) return 1 ;;
...
ui_safe() {
  # Default: preserve Unicode. If a terminal/dialog build renders emojis badly,
  # the user can run with TUINIX_ASCII_UI=1. This is a fallback, not a policy.
  local s="$*"
  if [[ "$TUINIX_ASCII_UI" != "1" ]]; then
    printf '%s' "$s"
    return
  fi
  printf '%s' "$s" | sed \
...
record_dialog_label() {
  local id="$1"
  local scope="${2:-file}"
  local mark section meta dirty label
  mark="off"
  [[ "${REC_ENABLED[$id]}" == "1" ]] && mark="on"
  ...
  label+="$dirty"
  shorten "$(ui_safe "$label")" $(( TUINIX_WIDTH - 18 ))
}

collect_records_for_file() {
  local file="$1"
  local __out_var="$2"
  local out=()
  local id
...
# -----------------------------------------------------------------------------
# Optional tmux side pane
# -----------------------------------------------------tmux_preview_start------------------------

tmux_preview_available() {
  [[ "$TUINIX_TMUX_SIDE" == "1" ]] || return 1
  [[ -n "${TMUX:-}" ]] || return 1
  has_cmd tmux || return 1
  return 0
}
tmux_preview_start() {
  tmux_preview_available || return 1
  [[ -n "${TUINIX_TMUX_PANE_ID:-}" ]] && return 0

  if [[ -z "${TUINIX_TMUX_PREVIEW_FILE:-}" ]]; then
    TUINIX_TMUX_PREVIEW_FILE="/tmp/tuinix-preview.$$.txt"
  fi

  cat > "$TUINIX_TMUX_PREVIEW_FILE" <<'EOF'
TUINIX Preview
==============

Selecione um arquivo, seção ou pacote para ver detalhes aqui.
EOF
  ...
  TUINIX_TMUX_PANE_ID="$(
    tmux split-window \
      ...
      -P \
      -F '#{pane_id}' \
      "bash -lc 'while true; do clear; cat \"\$1\" 2>/dev/null || true; sleep 0.4; done' _ '$TUINIX_TMUX_PREVIEW_FILE'" \
      2>/dev/null
  )"
  local rc=$?
  set -e

  [[ $rc -eq 0 && -n "$TUINIX_TMUX_PANE_ID" ]]
}

tmux_preview_update() {
  tmux_preview_available || return 0

  if [[ -z "${TUINIX_TMUX_PREVIEW_FILE:-}" ]]; then
    TUINIX_TMUX_PREVIEW_FILE="/tmp/tuinix-preview.$$.txt"
  fi

  tmux_preview_start || return 0
  cat > "$TUINIX_TMUX_PREVIEW_FILE"
}


tmux_preview_stop() {
  [[ -n "${TUINIX_PREVIEW_PANE:-}" ]] || return 0
  tmux kill-pane -t "$TUINIX_PREVIEW_PANE" 2>/dev/null || true
  TUINIX_PREVIEW_PANE=""
}
tmux_preview_update() {
  tmux_preview_available || return 0
...
tmux_preview_for_file() {
  local file="$1"
  rebuild_stats
  {
    printf 'TUINIX side pane\n'
    printf '%s\n' '================'
    printf 'File: %s\n' "$(basename_safe "$file")"
    printf 'Active: %s/%s\n' "${FILE_ACTIVE[$file]:-0}" "${FILE_TOTAL[$file]:-0}"
    printf 'Dirty: %s\n\n' "$([[ "${FILE_DIRTY[$file]:-0}" == "1" ]] && printf yes || printf no)"
    printf 'Keys/buttons in main pane:\n'
    printf '  OK/Aplicar : apply checklist\n'
    printf '  Ações      : file/section actions\n'
    printf '  NixOS      : open package page\n'
    printf '  ESC/Voltar : previous screen\n\n'
    printf 'Sections:\n'
    local sections=() s key
    collect_sections_for_file "$file" sections
    for s in "${sections[@]}"; do
      key="$file|$s"
      printf '  %-28s %s/%s\n' "$(shorten "$s" 28)" "${SECTION_ACTIVE[$key]:-0}" "${SECTION_TOTAL[$key]:-0}"
    done
  } | tmux_preview_update
}
...
open_url() {
  ...
  if [[ -n "$opener" ]]; then
    ...
    TUINIX_LAST_STATUS="$(style_ok "Abrindo navegador: $url")"
  else
...
open_nixos_package() {
  local pkg="$1"
  [[ -n "$pkg" ]] || return 1
  open_url "$(nixos_search_url "$pkg")"
}

record_id_from_dialog_help_output() {
  local output="$1"
  local __out_var="$2"
  local line candidate id

  while IFS= read -r line; do
    [[ -n "$line" ]] || continue
    # dialog may return the tag, the item-help text, or implementation-specific
    # help text. Support numeric record ids and package names.
    if [[ "$line" =~ ^[0-9]+$ ]]; then
      candidate="$((10#$line))"
      if [[ -n "${REC_PKG[$candidate]+x}" ]]; then
        printf -v "$__out_var" '%s' "$candidate"
        return 0
      fi
    fi
    for id in "${!REC_PKG[@]}"; do
      if [[ "$line" == "${REC_PKG[$id]}" || "$line" == *"${REC_PKG[$id]}"* ]]; then
        printf -v "$__out_var" '%s' "$id"
        return 0
      fi
    done
  done <<< "$output"
...
choose_package_to_open() {
  ...
  if [[ -n "$section" ]]; then
    ...
    collect_records_for_file "$file" ids
  fi
  [[ ${#ids[@]} -gt 0 ]] || return 1
  ...
  for id in "${ids[@]}"; do
    if [[ -n "$section" ]]; then
      ...
      label="[${REC_SECTION[$id]}] ${REC_PKG[$id]}"
    fi
    args+=("$id" "$(shorten "$(ui_safe "$label")" $(( TUINIX_WIDTH - 14 )))")
  done

  local selected rc
  if run_dialog_capture selected \
    --no-tags \
    --cancel-label "Voltar" \
    --menu "Escolha um pacote para abrir no search.nixos.org" \
    "$TUINIX_HEIGHT" "$TUINIX_WIDTH" $(( TUINIX_HEIGHT - 8 )) \
    "${args[@]}"; then
    rc=0
  else
  ...
  case "$rc" in
    0) open_nixos_package "${REC_PKG[$selected]}" ;;
    1|255) return 1 ;;
    *) return 1 ;;
  esac
}
...
# -----------------------------------------------------------------------------
# V2 main flow: file -> packages first
# -----------------------------------------------------------------------------
...
ui_main_loop() {
  while true; do
    ...
    local choice
    choice="$(choose_one "Arquivos / ações" "${choices[@]}")" || continue
...
ui_file_packages() {
  ...
  while true; do
    ...
    collect_records_for_file "$file" ids
    if [[ ${#ids[@]} -eq 0 ]]; then
      TUINIX_LAST_STATUS="$(style_warn "Nenhum pacote editável em $(basename_safe "$file").")"
      return
    fi

    tmux_preview_for_file "$file"

    local selected_ids rc
    if run_package_checklist selected_ids "$file" "" ids; then
      rc=0
    else
      rc=$?
    fi
    ...
    case "$rc" in
      0)
        apply_selected_ids ids "$selected_ids"
        TUINIX_LAST_STATUS="$(style_ok "Seleção atualizada em memória.")"
        ;;
      1|255)
        return
      ...
      2|4)
        local rid=""
        if record_id_from_dialog_help_output "$selected_ids" rid; then
          open_nixos_package "${REC_PKG[$rid]}"
        else
          choose_package_to_open "$file" || true
        fi
        ;;
      3)
        ui_file_actions "$file"
        ;;
      *)
        return
        ;;
...
run_package_checklist() {
  ...
  local -a args=()
  local id label status help first_id=""
  local scope="file"
  [[ -n "$section" ]] && scope="section"

  for id in "${__ids_ref[@]}"; do
    [[ -z "$first_id" ]] && first_id="$id"
    label="$(record_dialog_label "$id" "$scope")"
    status="off"
    [[ "${REC_ENABLED[$id]}" == "1" ]] && status="on"
    help="${REC_PKG[$id]}"
    args+=("$id" "$label" "$status" "$help")
  done

  local title prompt active total dirty section_part
  total="${#__ids_ref[@]}"
  active=0
  for id in "${__ids_ref[@]}"; do
  ...
  dirty=""
  [[ "${FILE_DIRTY[$file]:-0}" == "1" ]] && dirty=" *"
  section_part=""
  [[ -n "$section" ]] && section_part=" > $(ui_safe "$section")"

  title="$(basename_safe "$file")${section_part}${dirty}"
  prompt="$(ui_safe "$title")\n\nPacotes primeiro. Marque/desmarque com mouse, espaço ou setas.\nAplicar salva em memória; Ações abre operações; NixOS abre search.nixos.org.\nAtivos nesta tela: $active/$total"

  local __rc
  if run_dialog_capture "$__out_var" \
    --no-tags \
    --separate-output \
    --ok-label "Aplicar" \
    --cancel-label "Voltar" \
    ...
    --help-label "NixOS" \
    --item-help \
    --default-item "$first_id" \
    --checklist "$prompt" \
    "$TUINIX_HEIGHT" "$TUINIX_WIDTH" $(( TUINIX_HEIGHT - 9 )) \
    "${args[@]}"; then
    __rc=0
  else
    __rc=$?
...
ui_file_actions() {
  ...
  while true; do
    rebuild_stats
    local choices=(
      "🔎 Filtrar pacote neste arquivo"
      "🧩 Escolher seção"
      "✅ Ativar todos deste arquivo"
      "🚫 Desativar todos deste arquivo"
      "↩️ Reverter arquivo"
      "🌐 Abrir pacote no NixOS Search"
      "📊 Diff deste arquivo"
      "💾 Salvar este arquivo"
      "🪟 Alternar preview tmux"
      "← Voltar"
    )

    local choice
    choice="$(choose_one "Ações de $(basename_safe "$file")" "${choices[@]}")" || return

    case "$choice" in
      "🔎 Filtrar pacote neste arquivo") ui_file_search "$file" ;;
      "🧩 Escolher seção") ui_section_picker_v2 "$file" ;;
      "✅ Ativar todos deste arquivo") set_file_enabled "$file" 1 ;;
      "🚫 Desativar todos deste arquivo") set_file_enabled "$file" 0 ;;
      "↩️ Reverter arquivo") revert_file "$file" ;;
      "🌐 Abrir pacote no NixOS Search") choose_package_to_open "$file" || true ;;
      "📊 Diff deste arquivo") ui_diff_file "$file" ;;
      "💾 Salvar este arquivo") ui_save_file "$file" ;;
      "🪟 Alternar preview tmux") toggle_tmux_preview ;;
      "← Voltar") return ;;
      *) return ;;
    esac
  done
...
set_file_enabled() {
  ...
  TUINIX_LAST_STATUS="$(style_ok "Arquivo atualizado em memória.")"
}
...
revert_file() {
  ...
  TUINIX_LAST_STATUS="$(style_ok "Arquivo revertido para o estado original.")"
}

toggle_tmux_preview() {
  if [[ "$TUINIX_TMUX_SIDE" == "1" ]]; then
    TUINIX_TMUX_SIDE=0
    tmux_preview_stop || true
    TUINIX_LAST_STATUS="$(style_ok "Preview tmux desativado.")"
  else
    TUINIX_TMUX_SIDE=1
    if tmux_preview_available; then
      TUINIX_LAST_STATUS="$(style_ok "Preview tmux ativado.")"
    else
      TUINIX_LAST_STATUS="$(style_warn "Preview tmux requer estar dentro do tmux e ter tmux no PATH.")"
    fi
  fi
}

ui_section_picker_v2() {
  local file="$1"
  local sections=()
  collect_sections_for_file "$file" sections
  [[ ${#sections[@]} -gt 0 ]] || return
  ...
  local choice matched
  choice="$(choose_one "Seções de $(basename_safe "$file")" "${choices[@]}")" || return
  ...
  [[ -n "$matched" ]] && ui_section_packages_v2 "$file" "$matched"
}

ui_section_packages_v2() {
  local file="$1"
  local section="$2"
  local ids=()
  ...
  while true; do
    ...
    tmux_preview_for_section "$file" "$section"

    local selected_ids rc
    if run_package_checklist selected_ids "$file" "$section" ids; then
      rc=0
    else
      rc=$?
    fi
    ...
    case "$rc" in
      ...
      2|4)
        ...
        if record_id_from_dialog_help_output "$selected_ids" rid; then
          ...
          choose_package_to_open "$file" "$section" || true
        fi
        ;;
      3)
        ui_section_actions_v2 "$file" "$section"
        ;;
      *) return ;;
    esac
  done
...
tmux_preview_for_section() {
  ...
  collect_records "$file" "$section" ids
  {
    ...
    printf 'Packages:\n'
    for id in "${ids[@]}"; do
      printf '  %s %s\n' "$([[ "${REC_ENABLED[$id]}" == "1" ]] && printf '[x]' || printf '[ ]')" "${REC_PKG[$id]}"
    done
  } | tmux_preview_update
}

ui_section_actions_v2() {
  local file="$1"
  local section="$2"
  local choices=(
    "🔎 Filtrar nesta seção"
  ...
  local choice
  choice="$(choose_one "Ações: $(basename_safe "$file") > $section" "${choices[@]}")" || return
  case "$choice" in
    "🔎 Filtrar nesta seção") ui_section_filter "$file" "$section" ;;
    "✅ Ativar todos da seção") set_section_enabled "$file" "$section" 1 ;;
    "🚫 Desativar todos da seção") set_section_enabled "$file" "$section" 0 ;;
    "↩️ Reverter seção") revert_section "$file" "$section" ;;
    "🌐 Abrir pacote no NixOS Search") choose_package_to_open "$file" "$section" || true ;;
    "📊 Diff deste arquivo") ui_diff_file "$file" ;;
    "💾 Salvar este arquivo") ui_save_file "$file" ;;
    *) ;;
  esac
...
# Keep the old function names as compatibility shims. Any path that still calls
# ui_sections/ui_packages now lands on the new package-first screens.
ui_sections() {
  ui_file_packages "$1"
}

ui_packages() {
  ui_section_packages_v2 "$1" "$2"
}
...
ui_record_action() {
  ...
  local tmp="$TUINIX_TMPDIR/record-$id.txt"
  {
    printf 'Pacote : %s\n' "$pkg"
    printf 'Arquivo: %s\n' "$(basename_safe "$file")"
    printf 'Seção  : %s\n' "$section"
  ...
  while true; do
    local choices=(
      ...
      "🚫 Desativar"
      "↩️ Reverter"
      "🌐 Abrir no NixOS Search"
      "📄 Ver detalhes"
      "← Voltar"
    )
    local choice
    choice="$(choose_one "Pacote: $pkg" "${choices[@]}")" || return
    case "$choice" in
      "🔁 Alternar") toggle_record "$id" ;;
      "✅ Ativar") REC_ENABLED[$id]=1 ;;
      "🚫 Desativar") REC_ENABLED[$id]=0 ;;
      "↩️ Reverter") REC_ENABLED[$id]="${REC_ENABLED_ORIG[$id]}" ;;
      "🌐 Abrir no NixOS Search") open_nixos_package "$pkg" ;;
      "📄 Ver detalhes") pager_text "Pacote: $pkg" "$tmp" ;;
      "← Voltar") return ;;
      *) return ;;
    esac
  done
