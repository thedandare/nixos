...
# 2. Resolução Dinâmica de Interface de Rede (Guarda do Eth)
echo "🔍 [1/6] Capturando endereçamento na malha SDN Eth..."
LOCAL_TS_IP=$(ip -4 addr show dev eth0 2>/dev/null | grep -oP '(?<=inet )\d+(\.\d+){3}' || true)

if [ -z "$LOCAL_TS_IP" ]; then
    echo "❌ Erro Crítico: Interface 'eth0' não foi localizada no barramento de rede."
    echo "💡 Certifique-se de que o script 'autoconnect-tailscale.sh' concluiu o handshake."
    exit 1
fi
echo "✅ IP Local Eth validado: $LOCAL_TS_IP"
...
if ! ping -c 2 -W 3 "$PRIMARY_LEADER_IP" >/dev/null 2>&1; then
    echo "❌ Erro: Não há rota até o nó líder ($PRIMARY_LEADER_IP) via túnel Eth."
    echo "🔹 Verifique as ACLs do Eth ou o status de conexão da máquina principal."
    exit 1
fi
...
# #!/bin/bash
      ...
      #
      # echo "🔍 Capturando endereçamento de rede local na interface eth0..."
      # LOCAL_TS_IP=$(ip -4 addr show dev eth0 2>/dev/null | grep -oP '(?<=inet )\d+(\.\d+){3}' || true)
      #
      # if [ -z "$LOCAL_TS_IP" ]; then
      #     echo "❌ Erro Crítico: Interface 'eth0' indisponível."
      #     exit 1
      # fi
      #
