        ...
        4)
            ...
            case "$ACAO" in
                ...
                3)
                    ...
                    # Cria e limpa o buffer de IPs livres usando a variavel global do PID pai
                    TMP_LIVRES="/tmp/ts_livres.$$"
                    : > "$TMP_LIVRES"

                    # Varredura logica em memoria RAM usando o JSON ja carregado
                    ...
                    if [ ! -s "$TMP_LIVRES" ]; then
                        whiptail --title 'Aviso' --msgbox 'Todos os blocos de endereçamento respondem na sub-rede.' 8 55 3>&1 1>&2 2>&3
                        rm -f "$TMP_LIVRES"
                        TELA=3; continue
                    fi

                    # Abre o menu de alocacao aproveitando os descritores estaveis do script pai
                    IP_SELECIONADO=$(xargs -a "$TMP_LIVRES" whiptail --title 'Enderecos Livres Identificados' \
                        --ok-button 'Adotar IP' \
                        --cancel-button 'Voltar' \
                        --menu "Selecione o IP disponivel na faixa $BASE_IP.0/24 para o cluster:" \
                        22 75 12 3>&1 1>&2 2>&3)
                    STATUS_LIVRES=$?
                    rm -f "$TMP_LIVRES"
                    ...
                    if [ "$STATUS_LIVRES" -eq 0 ] && [ -n "$IP_SELECIONADO" ]; then
                        ...
                        # Captura o ID correto correspondente ao IP da escolha anterior ($CHOICE)
                        ...
                        whiptail --title 'Sucesso' --msgbox "$INFO_SUCESSO" 13 70 3>&1 1>&2 2>&3

                        # Dispara a chamada de API parametrizada com o espacamento obrigatorio de pontos
                        ./tailscale_set_static_ip.sh -i "$IP_SELECIONADO" -d "$SELECTED_DEVICE_ID" -t "$API_TOKEN"

                        if [ $? -eq 0 ]; then
                            TELA=0 # Sucesso absoluto. Finaliza o script inteiro e libera o provisionador
                        else
                            whiptail --title 'Erro na API' --msgbox 'Falha ao sincronizar o novo IP.' 8 60 3>&1 1>&2 2>&3
                            TELA=3
                        fi
                    else
                        TELA=3
...
clear
echo "[SUCCESS] Painel encerrado de forma sanitaria. Ambiente restaurado."
